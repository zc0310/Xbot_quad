...
namespace ecl
{
  ...
  public:
    ...
    /**
     * @brief Write a character string to the serial port.将字符串写入串口
     *
     * This function gives a compile time error if the type is not a byte (refer to ecl_mpl's)
     * is_byte type traits.
     *
     * @param bytes : points to the beginning of the character string. //指向字符串的开头
     * @param n : the number of characters to write. //要写入的字符数量
     * @exception StandardException : throws reading returned an error [debug mode only].
     **/
    template <typename Byte>
