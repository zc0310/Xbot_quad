 ...
 *
 * @brief Device driver core interface. //设备驱动核心接口
 *
 * License: BSD
...
union union_sint16 //联合体，共占同一段内存
{
  short word;
 ...
 *****************************************************************************/
/**
 * @brief  The core xbot driver class. //xbot核心驱动类
 *
 * This connects to the outside world via sigslots and get accessors.
   通过sigslots连接到外部世界并获取访问器
 **/
...
public:
  ...
  bool isAlive() const { return is_alive; }/*a. const成员函数不被允许修改它所在对象的任何一个数据成员。
                                             b. const成员函数能够访问对象的const成员，而其他成员函数不可以。
                                             c. 该成员函数不能修改类中任何非const成员函数*/
  ...
  *******************************************/
  ...
  /******************************************
  ** Getters - User Friendly Api 用户友好界面
  *******************************************/
  ...
  ** Soft Commands
  **********************/
  void resetOdometry();    //重置里程计

  /*********************
  ** Hard Commands
  **********************/
  void setBaseControl(const float &linear_velocity_x, const float &linear_velocity_y, const float &angular_velocity); //设置线速度 角速度
  void setLiftControl(const unsigned char &height_percent); //升高控制
  void setPlatformCameraControl(const unsigned char &platform_degree, const unsigned char &camera_degree);//相机
  void resetXbot();
...
private:
  ...
  ** Record RobotState
  **********************/
  unsigned char HeightPercent; //高度
  unsigned char PlatformDegree;//平台角度
  unsigned char CameraDegree;  //相机角度
  ...
  ** Commands
  **********************/
  void sendBaseControlCommand();  //发送控制指令
  ...
  **********************/
  ...
  // should be const, but pushnpop is not fully realised yet for const args in the formatters.
