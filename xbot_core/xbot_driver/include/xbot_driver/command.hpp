...
public:
  ...
  enum Name
  {//标识符
    BaseControl = 3, Lift = 4, TurnPlatformAndCamera = 5, Distance = 6, Power = 1
  };
  ...
  {
    Data()
      : command(BaseControl), speed_x(0.0), speed_y(0.0), radius(0.0), lift_height(0),
        platform_angle(0), camera_angle(0), power_state(0), distance_x(0.0), distance_y(0.0)
    {
    }
  ...
  static Command SetVelocityControl(const float &speed_x, const float &speed_y, const float &radius);
