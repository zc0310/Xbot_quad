...
void velocityCommand(const double& vx, const double& vy, const double &wz) {
  const double epsilon = 0.0001; //阈值
  if ( std::abs(wz) < epsilon ) {
    radius = 0;
  } else if ( (std::abs(vx) < epsilon ) && (std::abs(vy) < epsilon ) && ( wz > epsilon ) ) {
    radius = 1;
  } else if ((std::abs(vx) < epsilon ) && (std::abs(vy) < epsilon ) && ( wz < -1*epsilon ) ) {
    radius = -1;
  } else {
